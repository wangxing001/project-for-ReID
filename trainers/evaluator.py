...
class ResNetEvaluator:
    ...
    def evaluate(self, queryloader, galleryloader, queryFliploader, galleryFliploader, 
        ...
        for inputs0, inputs1 in zip(queryloader, queryFliploader):
            ...
            inputs0_1, inputs0_2, pids, camids = self._parse_data(inputs0)
            # print('inputs0-1:', inputs0_1.shape, inputs0_1)
            # print('\n')
            # print('inputs0-2:', inputs0_2.shape, inputs0_2)
            feature0 = self._forward(inputs0_1, inputs0_2)
            if eval_flip:
                inputs1_1, inputs1_2, pids, camids = self._parse_data(inputs1)
                feature1 = self._forward(inputs1_1, inputs1_2)
        ...
        if re_ranking:
            ...
            distmat = q_g_dist 
        print("dismat: ", distmat.shape, distmat)

        # ------------- evaluate stage: only need read distmat --------------
    ...
    def _parse_data(self, inputs):
        imgs_1, imgs_2, pids, camids = inputs
        return imgs_1.cuda(), imgs_2.cuda(), pids, camids

    def _forward(self, inputs, inputs1):
        with torch.no_grad():
            feature = self.model(inputs, inputs1)
        return feature.cpu()
    ...
    def eval_func_gpu(self, distmat, q_pids, g_pids, q_camids, g_camids, max_rank=50):
        ...
        results = []
        num_rel = []
        print("matches and num_g: ", matches, " ", num_g)
        ...
        print("results: ",results) # results = [] 有问题
